...
local UpdateSkins = function()
    for _, v in pairs(ns.skin) do
        if v and v:GetObjectType() == "Texture" and v:GetVertexColor() then
            v:SetVertexColor(MODUI_VAR["theme"].r, MODUI_VAR["theme"].g, MODUI_VAR["theme"].b)
        end
    end
    for _, v in pairs(ns.skinb) do
        if v and v:GetBackdropBorderColor() then
            v:SetBackdropBorderColor(MODUI_VAR["theme"].r, MODUI_VAR["theme"].g, MODUI_VAR["theme"].b)
        end
    end
...
ColorPickerFrame.reset:SetScript(
    "OnClick",
    function()
        MODUI_VAR["theme"].r, MODUI_VAR["theme"].g, MODUI_VAR["theme"].b = 1, 1, 1
        ColorPickerFrame:SetColorRGB(MODUI_VAR["theme"].r, MODUI_VAR["theme"].g, MODUI_VAR["theme"].b)
        for _, v in pairs(ns.skin) do
            if v and v:GetObjectType() == "Texture" and v:GetVertexColor() then
                v:SetVertexColor(MODUI_VAR["theme"].r, MODUI_VAR["theme"].g, MODUI_VAR["theme"].b)
            end
...
local OnShow = function(self)
    for _, v in pairs(
        ...
        v:SetVertexColor(ns.colour[1], ns.colour[2], ns.colour[3])
    end
    self:SetBackdropBorderColor(ns.colour[1], ns.colour[2], ns.colour[3])
end
